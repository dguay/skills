...
REVIEW_HEAD=$(git rev-parse HEAD); REVIEW_STARTED_AT=$(date +%s)
"$WAIT_REVIEW" prepare --sentinel /tmp/volley-done-$SLUG --result /tmp/volley-review-$SLUG.json
codex sandbox --permissions-profile claude-review -C "$PWD" -- timeout 300 "$HOME/.local/bin/claude" -p --model sonnet --strict-mcp-config --mcp-config '{"mcpServers":{}}' --resume "$SESSION_ID" --output-format json "$SPEC_CONTEXT Fixes are in. The fix delta is in /tmp/volley-fixdiff-$SLUG.patch. Verify each of your prior findings is addressed by this delta — do NOT re-review the full change, do NOT run /code-review, do NOT spawn sub-agents. Flag a NEW finding only if the fix itself introduced it. End with VERDICT: APPROVED or VERDICT: REVISE." < /dev/null > /tmp/volley-review-$SLUG.json; echo "REVIEW_EXIT=$?" | tee /tmp/volley-done-$SLUG
```
