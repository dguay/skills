...
1. **Project CLAUDE.md / AGENTS.md** — look for a "check command", "verify", or "smoke" instruction. This wins over everything below.
...
If NOTHING is found: say so, propose the 1-2 commands that would apply, and ask the user to confirm once — then offer to write them into the project's CLAUDE.md so this never gets asked again.
...
- **Supabase edge functions / DB**: after deploy or migration, call `mcp__supabase__get_logs` and scan for errors from the relevant function. An error-free recent log window is required for GREEN.
...
- Never stop/kill a running dev server on your own (e.g. a build/smoke needs the port). If a check requires the dev server stopped, ask the user for approval first — do not `kill`/`pkill`/free the port unprompted.
